<template>
    <div class="group h-full flex flex-col justify-between h-full md:col-span-6"
        :class="[route.name !== 'articles' ? 'p-4 border border-gray-200 rounded-lg cursor-pointer' : 'border-b-2 border-[#F94B65] border-dashed pb-4']">
        <div>
            <!-- Title & date -->
            <div class="flex flex-col gap-2 px-3 py-2 border-l-3 border-[#F94B65] mb-4">
                <h1 :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']" class="text-base">{{ title
                    }}
                </h1>
                <span :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']"
                    class="text-[12px] font-semibold">{{ date }}</span>
            </div>

            <!-- Cover image -->
            <div class="my-4 flex justify-center">
                <div :class="[aspect === 'portrait' ? 'aspect-[0.71] w-50' : 'aspect-video']">
                    <img :src="cover_url" @load="checkAspect" ref="imageEl" class="h-full w-full rounded-lg"
                        alt=""></img>
                </div>
            </div>
            ...
            <div class="flex justify-between mb-4">
                <div class="flex items-center gap-2 py-0">
                    ...
                    <span :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']"
                        class="text-sm font-semibold tracking-wider">{{ author }}</span>
                </div>

                <div class="border-b-2 border-[#F94B65] pb-1">
                    <span :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']"
                        class="text-[12px] text-semibold text-[#F94B65]">Article</span>
                </div>
            </div>

            <p :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']"
                class="text-sm tracking-wider leading-[32px] line-clamp-3 mb-4">
                {{ description }}
            </p>
        </div>

        <div class="flex justify-end gap-4 mt-auto cursor-pointer">
            <slot></slot>
            <div :class="[route.name !== 'articles' ? 'text-gray-700' : 'text-white']"
                class="hover:bg-gray-600 hover:text-white transition  px-4 py-1 text-[12px] rounded-full border border-gray-300">
